...
public interface IKillOrderService {
    ...
    @RequestMapping(value = "/queryByPageKill", method = RequestMethod.POST,consumes = MediaType.APPLICATION_JSON_VALUE)
    JSONObject queryByPageKill(@RequestParam("type") Integer type,
                               @RequestParam("keywords") String keywords,
                               @RequestParam("userId") String userId,
                               @RequestParam("addTime") Long addTime,
                               @RequestParam("pageSize") int pageSize);
