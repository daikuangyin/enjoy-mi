...
/*    @Transactional
    ...
    public GridModel<Order> searchListPage(Integer type, String keywords, int page, int pageSize, String userId) {
        ...
        Long ntotal = orderMapper.queryByPageCount(type, keywords, userId, naddTime == 0 ? "" : naddTime, pageSize);
        //调用秒杀系统的查询接口
//        List<Order> killorders = iKillOrderService.queryByPage(type, keywords, userId, kaddTime, pageSize);
        JSONObject jsonObject = iKillOrderService.queryByPageKill(type, keywords, userId, kaddTime, pageSize);
        List<Order> killorders = JSONArray.parseArray(JSON.toJSON(jsonObject.get("rows")).toString(),Order.class);
        Long killtotal = Long.valueOf(jsonObject.get("total").toString());
        //对普通订单和秒杀订单的合并排序并且记录页码和addTime的关系
        List<Order> pageList = sortOrdersByTime(normalOrders, killorders, userId, page, pageSize);
        GridModel<Order> gm = new GridModel<>();
        gm.setRecords(ntotal + killtotal);
        gm.setRows(pageList);
        gm.setPage(page);
