...
//        redisTemplate.opsForValue().set(orderId+"",order);
    ...
    @Override
    public JSONObject queryByPageKill(Integer type, String keywords, String userId, Long addTime, int pageSize) {
        JSONObject pageMap = new JSONObject();
        List<Order> pageList = orderMapper.queryByPage(type, keywords, userId, addTime == 0 ? "" : addTime, pageSize);
        pageMap.put("rows", JSONObject.toJSONString(pageList));
        pageMap.put("total",orderMapper.queryByPageCount(type, keywords, userId, addTime == 0 ? "" : addTime, pageSize));
        return pageMap;
    }
